...
class DungTuongTheLaHay:
    def __init__(self, proxy=None):
        ...
        # Tạo đối tượng requests session với proxy
        self.session = requests.Session()
        if self.proxies:
            self.session.proxies.update(self.proxies)

        # Tạo đối tượng cfscrape scraper
    ...
    def VuHoangTaDayPro(self):
        ...
        request = self.scraper.get(self.base_url, headers=self.headers)
        ...
        try:
            ...
            request = self.scraper.get(f"{self.base_url}{captcha_url}", headers=self.headers)
            open('z_captcha_icetool_vip_pro.png', 'wb').write(request.content)
            print("\033[1;36mĐang Xử Lý...", end="\r")
            return False
        except Exception as e:
            if str(e) == "list index out of range": print(request.text)
            print(self.iceArrow + '\033[1;31mLỗi Khi Giải Captcha: ' + str(e))
            time.sleep(2)
            self.VuHoangTaDayPro()

    def TheySayGoSlow(self, new_session = False):
        if new_session: self.session = requests.Session(); self.session.proxies.update(self.proxies); os.remove('session'); time.sleep(2)
        ...
        request = self.scraper.post(self.base_url, headers=self.headers, data=self.captcha_)
    ...
    def Rebooted(self):
        request = self.scraper.get(self.base_url, headers=self.headers).text
    ...
    def March_Of_The_Oni(self):
        ...
        while True:
            ...
            request = self.scraper.post(f'{self.base_url}{self.services_ids[self.service]}', headers = {"Origin": "https://zefoy.com","User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36","X-Requested-With": "XMLHttpRequest",}, data = {self.video_key: self.url})
    ...
    def Crystalized(self):
        ...
        request = self.scraper.post(f'{self.base_url}{self.services_ids[self.service]}', headers = {"Origin": "https://zefoy.com","User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36","X-Requested-With": "XMLHttpRequest",}, data = {self.video_info[0]: self.video_info[1]})
